...
const IgdLabor = () => {
  ...
  useEffect(() => {
    const fetchDataDetailPemeriksaan = async () => {
      try {
        const response = await api.get(`/api/v1/templateLaboratorium?kdJenisPrw=${kodePemeriksaan}`)
  ...
  const pilihPemeriksaan = (kode: any) => {
    ...
    if (pilihPemeriksaanModal.current) {
      pilihPemeriksaanModal.current.open()
    }
  }

  const handleCloseSecondModal = () => {
    if (pilihPemeriksaanModal.current) {
      pilihPemeriksaanModal.current.close()
    }
    setLabSelectedDetailData([])
  }
    ...
    if (!indikasi) {
      ...
      if (isDataCorrect) {
        try {
          ...
          const response1 = await api.post('/api/v1/permintaanLab', dataPermintaanLab)
          setSending(true)
          setErrInfo('')
          ...
          for (const item of listSelectedDetailData) {
            ...
            try {
              await api.post('/api/v1/permintaanPemeriksaanLab', {
            ...
            for (const innerItem of item[key]) {
              try {
                const response3 = await api.post('/api/v1/permintaanDetailPermintaanLab', {
                  noOrder: response1.data.noorder,
                  kdJenisPrw: key,
      ...
      <div className='w-full p-3 bg-slate-100'>
        ...
        <div>
          {listSelectedDetailData.map((item, index) => (
            <div key={index} className='h-80 overflow-auto border border-slate-300 rounded-lg mt-4'>
                      ...
                      {item[key].map((innerItem, innerIndex) => (
                        ...
                        >
                          ...
                          <td className='text-start'>
                            LD: {innerItem.nilai_rujukan_ld}, LA: {innerItem.nilai_rujukan_la}, PA:{' '}
                            {innerItem.nilai_rujukan_pa}
                          </td>
                          <td>
                            <button
                              className='underline'
